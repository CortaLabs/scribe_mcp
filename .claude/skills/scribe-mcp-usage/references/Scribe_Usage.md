...
### `search`
**Purpose**: Multi-file codebase search with grep/rg feature parity. Searches across repository files using regex or literal patterns with file type filtering and multiple output modes. Respects repo boundaries.

**Required Parameters:**
- `agent` (string): Agent identifier for audit trail
...
| `context_lines` | int | 0 | Lines of context around each match (both before and after) |
...
|------|---------|----------|
| `content` | Matching lines with context | Inspecting actual matches |
| `files_with_matches` | File paths only | Finding which files contain a pattern |
| `count` | Match counts per file | Gauging pattern frequency |

**Examples:**

```python
# Find all TODO comments in Python files
search(agent="CoderAgent", pattern="TODO", type="py")

# Case-insensitive search for a function name
search(agent="CoderAgent", pattern="def handle_request", case_insensitive=True, type="py")

# Find files containing a pattern (paths only)
search(agent="CoderAgent", pattern="import asyncio", output_mode="files_with_matches", type="py")

# Search with context lines
search(agent="CoderAgent", pattern="raise ValueError", context_lines=3, type="py")

# Literal string search (not regex)
search(agent="CoderAgent", pattern="config[\"key\"]", regex=False, glob="*.py")

# Multiline search across line boundaries
search(agent="CoderAgent", pattern="class.*:\\n.*def __init__", multiline=True, type="py")

# Count occurrences across codebase
search(agent="CoderAgent", pattern="logger\\.", output_mode="count", type="py")

# Search specific directory
search(agent="CoderAgent", pattern="async def", path="tools/", type="py")
```

**Notes:**
- Respects repo boundaries by default; paths outside repo root are denied
- Binary files are skipped by default (`skip_binary=True`)
- Results are capped by `max_matches_per_file`, `max_total_matches`, and `max_files`
- Use `glob` for path patterns and `type` for language-based filtering

### `edit_file`
**Purpose**: Safe file editing with exact string replacement. Provides dry-run preview by default, enforces read-before-edit, and respects repo sandbox boundaries.
...
| `dry_run` | bool | **True** | Preview without writing. Must explicitly set `False` to commit. |
| `format` | string | `"readable"` | Output format: `readable`, `structured`, `compact` |

**Enforcement Rules:**
- **read-before-edit**: `read_file` MUST have been called on the target path in the current session before `edit_file` can be used. Failure raises `READ_BEFORE_EDIT_REQUIRED`.
- **dry_run default**: `dry_run=True` by default. You MUST pass `dry_run=False` to actually write changes.
- **Exact match only**: No regex support; `old_string` must match exactly.
- **Uniqueness**: If `replace_all=False` (default) and `old_string` appears more than once, the edit fails with `STRING_NOT_UNIQUE`. Use `replace_all=True` or provide more context in `old_string`.

**Error Codes:**

| Error Code | Cause |
|------------|-------|
| `READ_BEFORE_EDIT_REQUIRED` | `read_file` was not called on this path first |
| `STRING_NOT_FOUND` | `old_string` does not exist in the file |
| `STRING_NOT_UNIQUE` | `old_string` matches multiple locations (and `replace_all=False`) |
| `SANDBOX_VIOLATION` | Path is outside repo boundaries |
| `SESSION_REQUIRED` | No active session for the agent |
...
read_file(agent="CoderAgent", path="config/settings.py", mode="scan_only")

# Step 2: Preview the edit (dry_run=True is default)
edit_file(
    agent="CoderAgent",
    path="config/settings.py",
    old_string='DEFAULT_TIMEOUT = 30',
    new_string='DEFAULT_TIMEOUT = 60'
)
...
1. `read_file(path="...", mode="scan_only")` — inspect file structure
2. `read_file(path="...", mode="line_range", start_line=X, end_line=Y)` — read the target area
3. `edit_file(old_string="...", new_string="...")` — preview (dry_run default)
4. `edit_file(old_string="...", new_string="...", dry_run=False)` — commit
