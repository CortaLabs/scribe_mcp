...
def _import_torchvision() -> Any:
    """Import torchvision lazily so failures can be handled explicitly."""
...
def _disable_broken_torchvision(
    ...
    torchvision_importer: Callable[[], Any] | None = None,
) -> bool:
    """Disable transformers' torchvision backend when torchvision is installed but broken.

    Some environments expose torchvision metadata but fail at import-time due to
    torch/torchvision binary mismatches. That failure can bubble into text-model
    loading via transformers internals. We proactively mark torchvision as
    unavailable for transformers when import fails.
    """

    if import_utils_module is None:
        try:
            from transformers.utils import import_utils as import_utils_module
        except Exception:
            return False

    if not bool(getattr(import_utils_module, "_torchvision_available", False)):
        return False

    importer = torchvision_importer or _import_torchvision
    try:
        importer()
        return False
    except Exception as exc:
        setattr(import_utils_module, "_torchvision_available", False)
        plugin_logger.info(
            "Disabled torchvision backend for transformers due to import failure: %s",
            exc,
        )
        return True
...
def _load_vector_deps() -> bool:
    ...
    try:
        ...
        plugin_logger.debug("Vector dependency load skipped: %s", exc)
        return False
...
class _DeterministicSentenceEncoder:
    """Lightweight fallback encoder when SentenceTransformer cannot load."""
    ...
    def encode(
        ...
        if isinstance(texts, str):
            ...
            entries = [str(item) for item in (texts or [])]

        vectors = np.zeros((len(entries), self.dimension), dtype="float32")
        for idx, text in enumerate(entries):
            digest = hashlib.sha256(text.encode("utf-8", errors="ignore")).digest()
            material = bytearray()
            counter = 0
            while len(material) < self.dimension * 4:
                seed = digest + counter.to_bytes(4, "little", signed=False)
                material.extend(hashlib.sha256(seed).digest())
                counter += 1

            raw = np.frombuffer(bytes(material[: self.dimension * 4]), dtype=np.uint32)
            vector = (raw.astype("float32") / float(2**32 - 1)) * 2.0 - 1.0
...
class VectorIndexer(HookPlugin):
    ...
    def _init_embedding_model(self) -> None:
        ...
        try:
            ...
            plugin_logger.warning(
                "Failed to load embedding model '%s': %s. Using deterministic fallback encoder.",
                model_name,
                e,
            )
            self.embedding_model = _DeterministicSentenceEncoder(self.vector_config.dimension)
    ...
    def get_index_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'fallback_encoder': self._using_fallback_encoder,
        }
