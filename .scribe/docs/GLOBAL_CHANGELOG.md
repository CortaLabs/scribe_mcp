...
## manage_docs host schema enrichment + case status/path correctness
- `source_project`: scribe_refinement_audit
- `source_entry_id`: 20260617:manage-docs-schema-keystone-case-correctness-2-8-0
- `summary`: Release 2.8.0 enriches the host-facing `manage_docs` MCP input schema and repairs bug/case correctness. (a) Schema enrichment (additive MCP capability): `manage_docs` now advertises a live `action` enum sourced at build time from the action manifest (`build_manage_docs_action_manifest()`), documents the generic frontmatter metadata keys plus per-action metadata sub-keys (e.g. `replace_text` find/replace/match_mode/replace_all/scope/allow_no_match, `replace_range` line_reference, `frontmatter_update` frontmatter, `status_update` status/proof), declares `required: ["action"]` to fix the auto-built required/default mismatch, and preserves `additionalProperties: true`. It reuses the existing `set_project` input_schema override path rather than introducing a parallel mechanism. (b) WS1 case correctness (F1/F2): unified the bug/case status vocabulary so closure-state statuses (including wontfix and duplicate) are preserved and recognized consistently, and replaced substring case-path resolution with exact (non-substring) matching that refuses to act on ambiguous case-path resolution. No breaking public API/CLI/schema removals (MINOR).

## Managed-doc registration, lean plugin shipping, and clean-room publish repair
- `source_project`: scribe_refinement_audit
- `source_entry_id`: 20260618:registration-lean-plugin-cleanbuild-2-8-1
- `summary`: Release 2.8.1 fixes managed-doc registration friction by auto-registering existing physical managed docs for targeted `manage_docs` actions, preserving persisted registry state, safely handling path-like registration keys, and preventing basename alias collisions. It fixes clean-checkout plugin sync so GitHub cleanbuild/publish no longer treats absent ignored generated skill trees as a request to prune shipped skills. The shipped plugin/wheel surface is intentionally lean: Claude and Codex bundles ship only `scribe-integration` and `scribe-onboarding`; legacy broad-form `scribe-mcp-usage` content and the old packaged onboarding copy are not shipped. Codex projection writes every shipped skill from the packaged bundle, clean-room install proof verifies the built wheel/runtime version/bundled skills/projection output, and managed-doc status `complete` is terminal for open-case filtering. No breaking public API/CLI/schema contract changes (PATCH).
