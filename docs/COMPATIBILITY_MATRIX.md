...
Release framing: **v2.2.8 compatibility baseline**

## Baseline contract

This baseline keeps compatibility posture for the 2.2.8 documentation wave:

- **Core package version:** `scribe-mcp==2.2.8`
- **Optional template package:** `scribe-council==2.2.8`
...
| --- | --- | --- | --- |
| `2.2.8` | — | Supported | Standard install for local/core usage. |
| `2.2.8` | `2.2.8` | Supported | Add optional template bundle when template assets are needed. |

## Not supported by this baseline

| Combination / practice | Status | Why |
| --- | --- | --- |
| `scribe-council` without `scribe-mcp==2.2.8` | Unsupported | Optional package explicitly depends on `scribe-mcp==2.2.8`. |
