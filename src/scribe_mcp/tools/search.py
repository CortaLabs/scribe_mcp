...
def _search_file(
    ...
    for idx in match_indices:
        ...
        if before > 0:
            ...
            ctx_before = [_truncate_line(all_lines[j].rstrip()) for j in range(start, idx)]

        if after > 0:
            end = min(total_lines, idx + after + 1)
            ctx_after = [_truncate_line(all_lines[j].rstrip()) for j in range(idx + 1, end)]
...
def _search_file_multiline(
    ...
    for m in compiled_pattern.finditer(text):
        ...
        matches.append(Match(line_number=line_number, line=_truncate_line(first_line)))
...
def _format_search_readable(data: Dict[str, Any], line_numbers: bool) -> str:
    ...
    if "pagination" in data:
        p = data["pagination"]
        parts.append(f"📄 Page {p['page']}/{p['total_pages']}")
        
        start_match = (p['page'] - 1) * p['page_size'] + 1
        end_match = min(p['page'] * p['page_size'], p['total_matches'])
        
        parts.append(f"   Showing matches {start_match}-{end_match} of {p['total_matches']} total")
        
        if p['has_next']:
            parts.append(f"   → Use page={p['page'] + 1} to see next {p['page_size']} matches")
        if p['has_prev']:
            parts.append(f"   ← Use page={p['page'] - 1} to see previous matches")
...
async def search(
    ...
    Parameters:
        ...
        page_size: Number of matches per page (default 10, min 1, max 100)
    ...
    # --- Pagination logic (TP-3.2) ---
    # Flatten all matches across files for pagination
    all_matches = []
    for file_result in results:
        for match in file_result.matches:
            all_matches.append({"file": file_result.file, "match": match})

    total_matches_for_pagination = len(all_matches)
    total_pages = (total_matches_for_pagination + page_size - 1) // page_size if total_matches_for_pagination > 0 else 1

    # Clamp page to valid range
    if page > total_pages:
        page = total_pages

    # Slice matches for requested page
    start_idx = (page - 1) * page_size
    end_idx = start_idx + page_size
    paginated_matches = all_matches[start_idx:end_idx]

    # Rebuild FileResult structure from paginated matches
    ...
    paginated_results = [
        FileResult(file=str(fpath), matches=matches, match_count=len(matches))
    ...
    pagination_info = {
        "page": page,
        "page_size": page_size,
        "total_matches": total_matches_for_pagination,
        "total_pages": total_pages,
        "has_next": page < total_pages,
        "has_prev": page > 1,
    }

    # Replace results with paginated results for output
    ...
    # Add pagination metadata to structured result
    structured["pagination"] = pagination_info
