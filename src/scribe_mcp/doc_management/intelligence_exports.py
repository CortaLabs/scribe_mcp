...
from scribe_mcp.doc_management.lifecycle import (
    derive_canonical_doc_type,
...
REJECTION_CODES = {
    ...
    "missing_policy": "REJECTED_MISSING_EXPORT_POLICY",
    "secret_suspect": "REJECTED_SECRET_SUSPECT",
    "broader_aggregate_policy": "REJECTED_BROADER_AGGREGATE_POLICY",
    "authority_mismatch": "REJECTED_POLICY_AUTHORITY_MISMATCH",
    "unstable_snapshot": "REJECTED_UNSTABLE_DOCUMENT_SNAPSHOT",
}

POLICY_AUTHORITY_FILE = INDEX_DIR / "export_policy_authority.json"

REQUIRED_EXPORT_POLICY_FIELDS = (
    "visibility",
    "owner_principal_id",
    "council_id",
    "project_id",
...
SECRET_SUSPECT_RE = re.compile(
    r"(?ix)(?:api[_-]?key|secret|password|token|aws_access_key_id|aws_secret_access_key|private[_-]?key|client[_-]?secret)\s*(?:[:=]|is)\s*[\"']?(?:AKIA[0-9A-Z]{16}|AIza[\w-]{35}|[A-Za-z0-9_\-\/+=]{8,})"
)
...
def _project_slug(active_project: Mapping[str, Any]) -> str:
    raw = (
        str(active_project.get("slug") or active_project.get("name") or "project")
        .strip()
        .lower()
    )
...
def _document_version_digest(
    doc_id: str, document_digest: str, policy_digest: str
) -> str:
    """Bind one export version to its server-computed document and policy facts."""
    return hashlib.sha256(
        b"scribe.document-version.v1"
        + doc_id.encode("utf-8")
        + document_digest.encode("utf-8")
        + policy_digest.encode("utf-8")
    ).hexdigest()


def _stable_snapshot(path: Path) -> tuple[bytes, Dict[str, Any], str] | None:
    """Read one identity-stable document image for every export decision."""
    try:
        before = path.stat()
        raw = path.read_bytes()
        after = path.stat()
    ...
    return raw, dict(metadata), parsed.body.strip()


def _load_policy_authority(root: Path) -> Mapping[str, Any]:
    path = root / POLICY_AUTHORITY_FILE
    try:
...
def _canonical_policy(policy: Mapping[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "owner_principal_id": str(policy.get("owner_principal_id") or "").strip(),
        "council_id": str(policy.get("council_id") or "").strip(),
        "project_id": str(policy.get("project_id") or "").strip(),
        "required_grants": sorted(
            str(grant).strip()
            for grant in policy.get("required_grants", [])
            if str(grant).strip()
        ),
        "revoked_at": policy.get("revoked_at"),
    }
...
def _export_policy_rejection(
    metadata: Mapping[str, Any],
    body: str,
    authority: Mapping[str, Any],
    path: str | None,
) -> str | None:
    if any(field not in metadata for field in REQUIRED_EXPORT_POLICY_FIELDS):
        return REJECTION_CODES["missing_policy"]
    if any(
        ...
        return REJECTION_CODES["missing_policy"]
    if not isinstance(metadata.get("required_grants"), list) or not all(
        str(grant).strip() for grant in metadata["required_grants"]
    ):
        return REJECTION_CODES["missing_policy"]
    if metadata.get("revoked_at") not in (None, "", False):
        return REJECTION_CODES["missing_policy"]
    if str(metadata.get("visibility")).strip().lower() != "knowledge":
        return REJECTION_CODES["missing_policy"]
    if str(metadata.get("policy_digest")) != _policy_digest(metadata):
        return REJECTION_CODES["missing_policy"]
    if SECRET_SUSPECT_RE.search(body):
        return REJECTION_CODES["secret_suspect"]
    registered = (
        authority.get(str(path or "")) if isinstance(authority, Mapping) else None
    )
    if not isinstance(registered, Mapping) or registered.get(
        "policy_digest"
    ) != metadata.get("policy_digest"):
        return REJECTION_CODES["authority_mismatch"]
    source_policies = metadata.get("source_policies")
    if source_policies is not None:
        if not isinstance(source_policies, list) or any(
            not isinstance(policy, dict) for policy in source_policies
        ):
            return REJECTION_CODES["broader_aggregate_policy"]
        document_policy = _canonical_policy(metadata)
        if any(
            _canonical_policy(policy) != document_policy for policy in source_policies
        ):
            return REJECTION_CODES["broader_aggregate_policy"]
    return None
...
def _is_curated_rollup(doc_meta: Mapping[str, Any]) -> bool:
    export_meta = (
        doc_meta.get("scribe_export")
        if isinstance(doc_meta.get("scribe_export"), dict)
        else {}
    )
...
def _heading_before_anchor(
    lines: List[str], anchor_line: int
) -> tuple[int, str] | None:
...
def _anchored_sections(*, body: str, fallback_title: str) -> List[Dict[str, Any]]:
    ...
    for section_index, (anchor_line, anchor_id) in enumerate(anchors):
        next_anchor_line = (
            anchors[section_index + 1][0]
            if section_index + 1 < len(anchors)
            else len(lines)
        )
        heading_before = _heading_before_anchor(lines, anchor_line)
        next_heading_before = (
            _heading_before_anchor(lines, next_anchor_line)
            if section_index + 1 < len(anchors)
            else None
        )
...
def _sections(*, body: str, fallback_title: str) -> List[Dict[str, Any]]:
...
def _knowledge_rows(
    *,
    ...
    for section in _sections(
        body=body, fallback_title=str(meta.get("title") or node["doc_name"])
    ):
        ...
        rows.append(
            {
                ...
                "required_grants": meta["required_grants"],
                "policy_digest": meta["policy_digest"],
                "revoked_at": meta["revoked_at"] or None,
                "document_digest": document_digest,
                "document_version_digest": document_version_digest,
...
def build_export_payload(*, active_project: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    policy_authority = _load_policy_authority(root)
    ...
    for item in items:
        ...
        canonical_type = derive_canonical_doc_type(
            meta.get("doc_type"), meta.get("intended_doc_type")
        )
        ...
        edge_map = (
            meta.get("topology") if isinstance(meta.get("topology"), dict) else {}
        )
        edges.extend(
            normalize_topology_edges(
    ...
    edges = sorted(
        ...
        key=lambda e: (
            ...
            str(e.get("target_ref") or ""),
            str(e.get("kind") or ""),
        ),
    )

    unresolved_sources = {
        str(e.get("source_doc_id")) for e in edges if not e.get("target_resolved")
    }
    manifest_records: List[Dict[str, Any]] = []
    knowledge_rows: List[Dict[str, Any]] = []
    exported_documents: List[Dict[str, str]] = []

    for n in nodes:
        reasons: List[str] = []
        quality = {
            "quality_status": "fail",
            "quality_summary": {"total_warnings": 0, "has_blockers": True},
        }
        if not n.get("path"):
        ...
        if (
            not n.get("doc_id")
            or not n.get("summary")
            or not n.get("canonical_doc_type")
        ):
        ...
        snapshot = _stable_snapshot(src) if src and src.exists() else None
        if snapshot:
            raw, snapshot_meta, body = snapshot
            n["metadata"] = snapshot_meta
            ...
            reasons.append(REJECTION_CODES["missing_quality"])
            if src:
                reasons.append(REJECTION_CODES["unstable_snapshot"])

        if _is_progress_like(
            doc_name=n["doc_name"], doc_type=n["canonical_doc_type"], path=n.get("path")
        ) and not _is_curated_rollup(n["metadata"]):
            reasons.append(REJECTION_CODES["raw_progress_log"])

        document_digest = ""
        if snapshot:
            policy_rejection = _export_policy_rejection(
                n["metadata"], body, policy_authority, n.get("path")
            )
            if policy_rejection:
        ...
        if eligible and snapshot:
            document_version_digest = _document_version_digest(
                n["doc_id"], document_digest, str(n["metadata"]["policy_digest"])
            )
            ...
            exported_documents.append(
                {
                    ...
                    "path": n["path"],
                    "document_digest": document_digest,
                    "policy_digest": str(n["metadata"]["policy_digest"]),
                    "document_version_digest": document_version_digest,
                }
            )
    ...
    manifest_records = sorted(
        manifest_records, key=lambda r: (r["path"] or "", r["doc_name"])
    )
    knowledge_rows = sorted(
        knowledge_rows,
        key=lambda r: (r["path"], r["doc_name"], r["section_index"], r["chunk_id"]),
    )
    ...
    return {
        "doc_topology": {
            "schema_version": "v1",
            "nodes": [
                {k: v for k, v in node.items() if k != "metadata"} for node in nodes
            ],
            "edges": edges,
        },
        "work_topology": {
        ...
        "downstream_ingestion_manifest": {
            ...
            "exported_documents": sorted(
                exported_documents,
                key=lambda record: (record["path"], record["doc_id"]),
            ),
...
def write_export_artifacts(*, active_project: Mapping[str, Any]) -> Dict[str, str]:
    ...
    manifest_path = out_dir / "downstream_ingestion_manifest.json"
    previous_exported_documents: List[Mapping[str, Any]] = []
    if manifest_path.exists():
        try:
            previous = json.loads(manifest_path.read_text(encoding="utf-8"))
            previous_exported_documents = (
                previous.get("exported_documents", [])
    ...
    current_ids = {
        record["doc_id"]
        for record in payload["downstream_ingestion_manifest"]["exported_documents"]
    }
    tombstones = [
        {
            "doc_id": str(record.get("doc_id") or ""),
            "path": str(record.get("path") or ""),
            "document_digest": str(record.get("document_digest") or ""),
            "policy_digest": str(record.get("policy_digest") or ""),
            "document_version_digest": str(record.get("document_version_digest") or ""),
            "tombstone_generation": hashlib.sha256(
                json.dumps(record, sort_keys=True, separators=(",", ":")).encode(
                    "utf-8"
                )
            ).hexdigest(),
        }
        for record in previous_exported_documents
        if isinstance(record, Mapping)
        and str(record.get("doc_id") or "") not in current_ids
    ]
    tombstones = sorted(
        tombstones, key=lambda record: (record["path"], record["doc_id"])
    )
    payload["knowledge_scribe_tombstones"] = tombstones
    payload["downstream_ingestion_manifest"]["tombstone_receipt"] = {
        "generated_count": len(tombstones),
        "residual_count": len(tombstones),
    }
    mapping = {
        ...
        "knowledge_scribe_tombstones": knowledge_out_dir
        / f"{project_slug}.tombstones.jsonl",
    }
    # Tombstone durable intent precedes any manifest that removes the document.
    tombstone_path = mapping["knowledge_scribe_tombstones"]
    tombstone_path.write_text(render_stable_jsonl(tombstones), encoding="utf-8")
    for key, path in mapping.items():
        if key == "knowledge_scribe_tombstones":
            continue
        if key in {"knowledge_scribe_export", "knowledge_scribe_tombstones"}:
