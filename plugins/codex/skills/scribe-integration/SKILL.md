...
owner: scribe-mcp
---
<!-- COUNCIL:GENERATED:BEGIN -->
<!-- Auto-generated by `council update` (generated by council update).
     Surface: skill | Path: scribe-integration/SKILL.md
     Do not edit by hand; edit the source template and regenerate. -->
...
Every discovered bug MUST be logged immediately in the active Scribe trail; never hide, defer, or silently absorb discovery. This immediate-visibility rule applies to every agent; it does not transfer Mantis's diagnosis/repair authority to other roles.

Tiny in-contract defects remain attached to the active work item and current Scribe trail and are corrected through the active owner/validation loop. They do not automatically require `open_bug`, a standalone report, or a `link_fix` lifecycle.

Open a durable case immediately when severity, recurrence, cross-session coordination, security significance, knowledge value, operator direction, or Atlas routing warrants it. Preserve Mantis's diagnosis/repair authority for unknown, intermittent, flaky, runtime, startup, state-sensitive, or recurrence-prone failures; this policy changes case activation and ceremony only, not tool or runtime semantics.

After the durable-case threshold is met, preserve the complete evidence, report, and fix-link lifecycle:

```python
# Step 1: Open the durable case
open_bug(
    agent="forge",
...
| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `open_bug` | additive | Open a durable bug case (per-day stable ID) and scaffold its report doc after the durable-case threshold is met. Every discovery is still logged immediately in the active Scribe trail. |
