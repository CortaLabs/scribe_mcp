...
**Version:** v1.0
**Status:** Approved
**Last Updated:** 2026-02-06 08:20 UTC
...
2. **No god modules** -- Every module under 800 lines; storage/ and doc_management/ are proper subpackages
...
|       |   |-- append_event.py
|       |   +-- manage_docs/       # DECOMPOSED from 3410-line god module
|       |       |-- __init__.py    # Entry point + action router
|       |       |-- actions/
...
### 4.2 Doc Management Decomposition (tools/manage_docs/)

29 functions split into action-based subpackage:

**__init__.py** (~100 lines): manage_docs() entry point, ACTION_ROUTER dict
**healing.py** (~200 lines): `_normalize_metadata_with_healing` (line 93), `_heal_manage_docs_parameters` (line 120), `_add_healing_info_to_response` (line 317), `_coerce_line_number` (line 243)
**validation.py** (~150 lines): Path safety, action validation, section ID validation
**preflight.py** (~100 lines): Backup creation with 3-backup retention, cleanup
**indexing.py** (~100 lines): `_chunk_text_for_vector` (line 337), `_generate_doc_entry_id` (line 402), `_get_index_updater_for_path` (line 2892), `_should_skip_doc_index` (line 431)
**utils.py** (~100 lines): `_hash_text` (line 332), `_parse_int` (line 465), `_split_into_sections` (line 341), `_split_section` (line 356), `_build_special_metadata` (line 777), `_resolve_custom_doc_path` (line 1023)
**actions/create.py** (~400 lines): Document creation (research, bug, custom, agent_card, review)
**actions/edit.py** (~400 lines): replace_section, replace_range, replace_text, apply_patch
**actions/append.py** (~100 lines): Append to doc/section
**actions/status.py** (~200 lines): status_update, checklist management
**actions/search.py** (~200 lines): Vector + text search
**actions/query.py** (~200 lines): list_sections, list_checklist_items, normalize_headers, generate_toc, validate_crosslinks
**actions/batch.py** (~100 lines): Multi-operation batch execution
...
4. Fix hardcoded paths (reminders.py line 31 uses wrong DB)
5. Back up and remove legacy DB files
...
| `tools/manage_docs.py` (3410 lines) | `tools/manage_docs/` (12 modules) | God module decomposition |
...
| Keep examples/council_bridge.py? | OPEN | Need product owner input |
...
- RESEARCH_STDERR_AUDIT_20260206_0755.md (115+ stderr sources)
