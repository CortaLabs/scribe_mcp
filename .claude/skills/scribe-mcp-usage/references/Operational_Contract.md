...
- All file reads must use `read_file` (scan/search/chunk/page/line_range). For multi-file codebase search use `search`. For safe edits use `edit_file` (requires prior `read_file`; `dry_run=True` by default). Do not read file contents with `cat`/`rg`; use `rg --files` only for filename discovery.
...
search(
  agent,              # REQUIRED
  pattern,            # REQUIRED - regex pattern (or literal if regex=False)
  path=None,          # Directory or file to search (default: repo root)
  glob=None,          # Glob pattern to filter files (e.g. "*.py", "src/**/*.ts")
  type=None,          # File type filter (py, js, ts, rust, go, java, etc.)
  output_mode="content",  # "content" (matching lines), "files_with_matches" (paths), "count" (counts)
  format="readable",
  context_lines=0,    # Lines of context around matches (both before and after)
  before_context=None,# Lines before match (overrides context_lines for before)
  after_context=None, # Lines after match (overrides context_lines for after)
  case_insensitive=False,
  regex=True,         # True for regex, False for literal string matching
  multiline=False,    # Enable multiline matching (pattern can span lines)
...
# Respects repo boundaries. 3 output modes: content, files_with_matches, count.

edit_file(
  agent,              # REQUIRED
  path,               # REQUIRED - file to edit (repo-relative or absolute)
  old_string,         # REQUIRED - exact string to find
  new_string,         # REQUIRED - replacement string
  replace_all=False,  # Replace all occurrences (default: first only)
  dry_run=True,       # Preview without writing (default: True for safety)
  format="readable"
)
# Safe file editing with exact string replacement.
# CRITICAL: read_file MUST have been called on this path in the current session.
# dry_run=True by default — must explicitly set False to commit changes.
# Error codes: READ_BEFORE_EDIT_REQUIRED, STRING_NOT_FOUND, STRING_NOT_UNIQUE,
#              SANDBOX_VIOLATION, SESSION_REQUIRED, WRITE_FAILED
```
...
- `read_file`: scan/search first; use `include_dependencies` only when needed; use `allow_outside_repo=true` for out-of-repo reads (denylist still enforced). `.claude/skills` and `.codex/skills` are always allowed.
 ...
 - `search`: multi-file codebase search; use `output_mode` to control results format; `regex=True` by default; respects repo boundaries.
 - `edit_file`: safe string replacement; always call `read_file` on the path first; `dry_run=True` by default — set `False` to commit.
