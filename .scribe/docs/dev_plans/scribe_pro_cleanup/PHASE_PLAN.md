...
**Version:** v1.0
**Status:** Approved
**Last Updated:** 2026-02-06 08:26 UTC
...
| P9: Reminder Wire-Up | 3 MCP tools, fix hardcoded DB path | 2 days | LOW | P7 |
| P10: Startup Optimization + Test Cleanup + Scaffold | Lazy loading, test fixtures, auth/transport interfaces | 3 days | LOW | P6 |

**Total Estimated Duration:** 31 days (~6-7 weeks with buffer)
...
**Exit Criteria:** Zero bare except clauses. Preflight backups bounded.
...
**Objective:** Decompose tools/manage_docs.py (3,410 lines, 29 functions) into tools/manage_docs/ subpackage.

**Duration:** 4 days | **Risk:** HIGH | **Dependencies:** P2 (logging)

### Task Package 5.1: Create Subpackage + Shared Modules

**Scope:** Create tools/manage_docs/ directory, extract healing, validation, utils
**Files to Create:** tools/manage_docs/__init__.py, tools/manage_docs/healing.py, tools/manage_docs/validation.py, tools/manage_docs/utils.py, tools/manage_docs/preflight.py, tools/manage_docs/indexing.py
...
### Task Package 5.2: Extract Action Modules

**Scope:** Create actions/ subdirectory with one module per action group
**Files to Create:** tools/manage_docs/actions/__init__.py, actions/create.py, actions/edit.py, actions/append.py, actions/status.py, actions/search.py, actions/query.py, actions/batch.py
...
### Task Package 5.3: Wire Up Router + Delete Old File

**Scope:** Create ACTION_ROUTER in __init__.py, delete old manage_docs.py
**Files to Modify:** tools/manage_docs/__init__.py, DELETE tools/manage_docs.py

**Specifications:**
1. manage_docs() in __init__.py: heals params, routes via ACTION_ROUTER dict
2. Include deprecated action routes (create_research_doc, create_bug_report, create_doc)
3. Delete old tools/manage_docs.py
4. Verify manage_docs_main() (used by server.py) still accessible

**Verification:**
- [ ] `wc -l tools/manage_docs/*.py tools/manage_docs/actions/*.py` -- all under 800
- [ ] `pytest tests/test_manage_docs*.py` -- FULL pass
- [ ] All 7 primary actions + deprecated routes work

**Exit Criteria:** tools/manage_docs.py eliminated. 12 focused modules, all under 800 lines.
...
### Task Package 6.3: Fix __file__ Path Resolutions

**Scope:** Replace 50+ __file__ patterns with config/paths.py calls
**Files to Modify:** server.py, config/settings.py, template_engine/engine.py, utils/reminder_monitoring.py, utils/tool_logger.py, utils/rotation_state.py, utils/audit.py, template_engine/cli.py, and ~42 more
...
**Exit Criteria:** `pip install -e .` works. All console scripts work. All tests pass. Zero __file__ in production code.
...
### Task Package 7.3: Fix Hardcoded DB Paths + Legacy Cleanup

**Scope:** Fix reminders.py hardcoded path, remove legacy DB files
**Files to Modify:** reminders.py (line 31), config/settings.py

**Specifications:**
1. Fix reminders.py line 31: replace hardcoded `.scribe/data/scribe.db` with settings.sqlite_path
...
4. Update config/settings.py default_db_path to use config/paths.py

**Verification:**
- [ ] `grep -rn '.scribe/data/scribe.db\|.scribe/scribe.db' src/ --include='*.py'` returns 0
- [ ] Reminder system works with centralized DB path
...
| Dead Code Clean (P3) | Week 2 | Coder | Planned | Zero bare except |
| Storage Decomposed (P4) | Week 2-3 | Coder | Planned | 9 modules, all < 800 lines |
| Docs Decomposed (P5) | Week 3-4 | Coder | Planned | 12 modules, all < 800 lines |
| Packaged + src/ (P6) | Week 4-5 | Coder | Planned | pip install works |
| DB Consolidated (P7) | Week 5 | Coder | Planned | Single DB, no state.json |
| Postgres Complete (P8) | Week 5-6 | Coder | Planned | 31/31 methods |
| Reminders Live (P9) | Week 6 | Coder | Planned | 3 tools in list_tools |
| Optimized + Scaffold (P10) | Week 6-7 | Coder | Planned | Startup < 2s |
