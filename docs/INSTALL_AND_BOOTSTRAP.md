...
This release line fixes readable `read_recent(compact=True)` output, adds internal `scribe_doctor` repo-local plugin diagnostics without production trust widening, improves `manage_docs` registration/drift diagnostics, adds advisory reminder guidance fields, accepts bounded single-managed-doc Codex-style `manage_docs(apply_patch)` input, and carries the public-safe read-only affected-row referential inventory preflight for governed project-row repair planning, faster repeated project binding, queryable runtime telemetry, append-entry phase timing, physical/logical reconciliation diagnostics, JSON probe output, same-server root comparison tooling, and background telemetry cleanup for local probes. Reminder hook/context injection remains security-blocked future work. It also includes the document topology foundation plus agent-ready quality governance: managed docs carry canonical lifecycle metadata, typed deterministic edges, quality-gated ready/complete transitions, topology/metadata scan actions, safe repair modes, stale cleanup recommendations, handoff checks, Atlas bulk quality checks, and sanitized downstream export manifests.
