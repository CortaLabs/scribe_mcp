...
2. **Audit and Logging**
   - Use `append_entry(agent="ResearchAgent", ...)` for every meaningful action or discovery.
...
5. **Index Enforcement**
   ...
   - If **three or more** reports exist:
     ...
     - The index must list:
       ...
       - Confidence summary
     - Log the index update with `append_entry(agent="ResearchAgent", status="info")` including `index_status="updated"`.
