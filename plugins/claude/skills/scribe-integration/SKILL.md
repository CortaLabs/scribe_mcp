...
> **This skill is THE canonical, complete reference for the Scribe MCP tool surface and workflow.** It documents every registered Scribe tool (see the [Full Tool Reference](#full-tool-reference) appendix), the mandatory startup sequence, the logging cadence, the document lifecycle, and bug/security reporting. If you are an agent or host that needs to discover or correctly call Scribe tools, this is the one-stop reference — re-open it any time with `/scribe-integration`. For first-time **install and connection** (getting Scribe running and reachable), see the separate `/scribe-onboarding` skill; this guide assumes Scribe is already connected.
...
This appendix enumerates **every** Scribe MCP tool the server registers, so an agent or host can discover the entire surface from this skill alone. The deep workflow sections above cover the core tools in detail; this table is the complete inventory. Tools are surfaced to hosts as `mcp__scribe__<name>` (the prefix depends on how the server is registered in your client).

**Trust tiers** come from each tool's contract wrapper and drive the MCP `annotations` a host sees:

| Tier | `annotations` | Meaning |
|------|---------------|---------|
| read-only | `readOnlyHint: true` | Reads/inspects only; never mutates state |
| additive (write) | `idempotentHint`/append semantics | Appends new records (logs, cases, events); does not rewrite existing state |
| stateful (write) | write, non-destructive | Mutates project/session/doc/reminder state in place |
| destructive (write) | `destructiveHint: true` | Removes or resets data; use deliberately |

> **Install/connection is NOT in this skill.** Getting Scribe installed and reachable (`pip install scribe-mcp`, the one-command local-Postgres wizard, env vars) lives in the separate `/scribe-onboarding` skill. Everything below assumes Scribe is already connected.

### Project & Context

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `set_project` | stateful | Bind the active project context for this session. **First call of every session.** Sticky — call once, not per log. |
| `get_project` | read-only | Return the active project selection (resolving defaults). Use to confirm what context you're bound to. |
| `list_projects` | read-only | List projects registered for the current repo (or all repos with `global_mode=True`). Use to find an existing workstream. |
| `delete_project` | destructive | Delete or archive a project and all associated data. Deliberate cleanup only. |
| `authorize_repo_root` | stateful | Issue a session-bound grant authorizing a bind to an external repository root. Needed before working outside the primary repo root. |

### Logging & Log Inspection

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `append_entry` | additive | Record an audit-trail log entry (single or bulk). **Log every 2-3 significant actions.** The primary "record what I did" tool. |
| `read_recent` | read-only | Load the most recent project log entries. **Second call of every session** (rehydrate before working). |
| `query_entries` | read-only | Search the project log history by message/term. Use to find prior work on a topic. |
| `progress_log_projection` | read-only | Public-safe readiness/render projection of the progress or doc-update log without mutating canonical logs. |
| `analyze_logs` | read-only | Log-intelligence analysis over a given log file path. Inspection only. |
| `rotate_log` | stateful | Rotate one or more project log files with integrity guarantees. Maintenance/admin. |

### Documents

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `manage_docs` | stateful | The document-governance router: create scaffolds, `replace_section`, `apply_patch`, `status_update`, `frontmatter_update`, `quality_check`, topology, changelog, and more. See the deep section above. |
| `generate_doc_templates` | additive | Render the standard project doc templates (architecture, phase plan, checklist, logs). Overwrites blocked unless `force=True`; progress logs always preserved. |

### Bugs, Security & Traceability

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `open_bug` | additive | Open a bug case (per-day stable ID) and scaffold its report doc. File immediately when you find a bug. |
| `open_security` | additive | Open a security case (per-day stable ID) and scaffold a detailed security report doc. |
| `list_open_cases` | read-only | List open bug/security cases from the shared case registry (filter by type/project/repo). |
| `link_fix` | additive | Link a fix artifact (`file.py:line`) to a case with a landing status (e.g. `merged`). |
| `append_event` | additive | Append a general sentinel event to `sentinel.jsonl` (append_entry-compatible args). Sentinel-mode logging. |

### File Reading & Search

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `read_file` | read-only | Structured code reading. **Always `scan_only` first**, then `line_range`/`chunk`/`page`/`search`/`full_stream`. See the deep section above. |
| `edit_file` | stateful | Safe exact-string file editing. Requires a prior `read_file` on the path this session; `dry_run=True` by default — set `False` to commit. |
| `search` | read-only | Regex search across the codebase with a `glob` filter. |

### Reminders

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `query_reminders` | read-only | Query reminder history and currently active reminders for a project. |
| `configure_reminders` | stateful | Configure reminder behavior (cadence/categories) for a project. |
| `reset_reminders` | destructive | Reset reminder cooldowns and/or reminder history for a project. |

### Diagnostics & Runtime (mostly internal)

| Tool | Tier | Purpose / When to use |
|------|------|------------------------|
| `scribe_doctor` | read-only | Runtime diagnostics for the current MCP server instance (config, roots, backend health). Run when Scribe behaves unexpectedly. |
| `read_write_barrier_state` | read-only | Public-safe Scribe write-barrier state for the active repo. |
| `scribe_owned_write_barrier_acquire_release_proof` | stateful | Acquire/release proof for the Scribe-owned write barrier. Internal coordination/proof tool. |
| `scribe_private_context_selector_readback` | read-only | Emit public-safe selector/readback labels without runtime or target contact. Internal proof tool. |
| `scribe_local_postgres_readiness_roundtrip_preflight` | read-only | Public-safe local-Postgres readiness roundtrip preflight. Internal readiness check. |

> `health_check` is also registered but is an internal liveness probe — hosts generally do not surface it for agent use.

### Quick usage for the useful-but-undocumented tools

The core workflow tools are covered in depth above. The snippets below cover the genuinely useful tools that the workflow sections don't already demonstrate.

```python
# Confirm / discover project context
get_project(agent="forge")                       # what am I bound to?
list_projects(agent="forge")                     # existing workstreams in this repo
list_projects(agent="forge", global_mode=True)   # across all repos

# Security case (parallel to open_bug)
open_security(
    agent="sentinel",
    title="Auth token logged in plaintext",
    symptoms="session_token appears in web_ui.log at INFO",
    severity="high",        # critical | high | medium | low
    category="data",
)
# then replace_section findings/severity/remediation, like a bug report

# See what cases are open
list_open_cases(agent="forge")                   # all open bug + security cases
list_open_cases(agent="forge", case_type="bug")  # filter by type

# Safe file edit (read_file first, dry_run preview, then commit)
read_file(agent="forge", path="src/auth.py", mode="line_range", start_line=140, end_line=160)
edit_file(agent="forge", path="src/auth.py",
          old_string="verify_exp=False", new_string="verify_exp=True",
          dry_run=True)   # preview; re-run with dry_run=False to commit

# Diagnose a misbehaving Scribe
scribe_doctor(agent="forge")                     # config, roots, backend health

# Reminders
query_reminders(agent="forge")                   # active reminders + history
configure_reminders(agent="forge", metadata={"stale_after_hours": 24})
```

**Tool count:** 30 registered tools (this appendix lists every one). The reference above is verified against the server's `@app.tool` registrations — if the surface changes, regenerate this skill from its `.j2` source rather than hand-editing the generated `.md`.
